...
{ 
  ...
  public partial class MatchingEngine
  {
    ...
    {
      return appPort.FqdnPrefix + findCloudletReply.Fqdn; // prepend fqdn prefix given in AppPort to fqdn
    }

    /*!
     * Returns the port of the developers app backend service based on the appPort provided.
     * An optional desiredPort parameter is provided if the developer wants a specific port within their appPort port range (if none provided, the function will default to the PublicPort field in the AppPort).
    ...
    private static bool AppPortIsEqual(AppPort port1, AppPort port2)
    {
      ...
      if (!port1.FqdnPrefix.Equals(port2.FqdnPrefix))
      {
        return false;
      }
      if (port1.InternalPort != port2.InternalPort)
      {
        return false;
      }
      ...
      if (!port1.PublicPort.Equals(port2.PublicPort))
      {
        return false;
      }
      return true;
    ...
    // Checks if the specified port is within the range of PublicPort and EndPort fields in AppPort
    private static bool IsInPortRange(AppPort appPort, int port)
    {
      int mappedEndPort = appPort.PublicPort + (appPort.EndPort - appPort.InternalPort);
      // Checks if range exists -> if not, check if specified port equals public port
      if (appPort.EndPort == 0 || mappedEndPort < appPort.PublicPort)
      {
    ...
    private static string GetAvailableInterfaceName(NetInterface netInterface, Regex interfaceNamesRegex)
    {
    ...
    {
      return GetAvailableInterfaceName(netInterface, networkInterfaceName.CELLULAR);
    }
    ...
    public Dictionary<int, AppPort> GetAppPortsByProtocol(FindCloudletReply reply, LProto proto)
    {
      ...
      RepeatedField<AppPort> ports = reply.Ports;
      foreach (AppPort port in ports)
      {
        if (port.Proto == proto)
        {
          appPortsByProtocol.Add(port.InternalPort, port);
        }
      }
    ...
    public Dictionary<int, AppPort> GetTCPAppPorts(FindCloudletReply reply)
    {
      ...
      foreach (AppPort port in ports)
      {
        ...
        {
          tcpAppPorts.Add(port.InternalPort, port);
        }
      }
      return tcpAppPorts;
    }
    ...
    public Dictionary<int, AppPort> GetUDPAppPorts(FindCloudletReply reply)
    {
      ...
      foreach (AppPort port in ports)
      {
        ...
        {
          udpAppPorts.Add(port.InternalPort, port);
        }
      }
      return udpAppPorts;
    }
