...
namespace Tests
{
  public class Tests
  {
    ...
    {
      var loc = await Util.GetLocationFromDevice();
      const int NOISE_THRESHOLD = 100; //Threshold for difference between net test averages
      FindCloudletReply reply1 = null;
    ...
    public async static Task TestRestartEdgeEventsConnection()
    {
      ...
      Loc bonnLoc = new Loc { Longitude = 50, Latitude = 13}; // San Jose.
      var findCloudletReply = await me.RegisterAndFindCloudlet(dmeHost, MatchingEngine.defaultDmeGrpcPort,
      ...
      bool locationSamplesSent = await me.EdgeEventsConnection.PostLocationUpdate(bonnLoc).ConfigureAwait(false);
      await Task.Delay(DELAY_TIME);
      ...
      locationSamplesSent = await me.EdgeEventsConnection.PostLocationUpdate(bonnLoc).ConfigureAwait(false);
      await Task.Delay(DELAY_TIME);
